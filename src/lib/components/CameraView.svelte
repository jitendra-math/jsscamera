<script>
    ...
    // Svelte 5 Props: Parent (+page.svelte) se function receive karna
    let { oncapture } = $props(); 
    ...
    // Component hatne par camera band karo
    onDestroy(() => {
        if (stream) {
            stopCamera(stream); //
        }
    ...
    function takePhoto() {
        ...
        try {
            ...
            // Svelte 5: Seedha parent ke callback function ko canvas pass karna
            if (oncapture) oncapture(canvas); 
        } catch (err) {
...
<div class="relative w-full h-[80vh] md:h-[600px] bg-neutral-900 overflow-hidden rounded-2xl shadow-2xl">
    ...
    <video 
        ...
        playsinline
        muted
    ></video> {#if !isCameraReady && !errorMessage}
